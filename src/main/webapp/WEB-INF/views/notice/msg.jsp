    ...
    <UL>
      <c:choose>
        ...
        <c:when test="${param.code == 'update_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">공지 수정에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        
        <c:when test="${param.code == 'delete_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">공지 삭제에 성공했습니다.</span>
          </LI>                                                                      
        </c:when>        
        
        <c:when test="${param.code == 'delete_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">공지 삭제에 실패했습니다.</span>
          </LI>                                                                      
        </c:when> 
      ...
      <LI class='li_none'>
        <br>
        <button type='button' onclick="location.href='./list_all.do'" class="btn btn-secondary btn-sm">리스트로 돌아가기</button>
      </LI>
    </UL>
  </fieldset>
