...
<script type="text/javascript">
  ...
  function send() {
    ...
    new_passwd2_msg.classList.remove('span_warning'); // 현재 패스워드 일치함으로 스타일 제거
    new_passwd2_msg.innerHTML=""; 
    ...
    .then(rdata => {
      if (rdata.cnt == 0) { // 패스워드 일치 X
        ...
        if (new_passwd.value != new_passwd2.value){
          ...
          new_passwd.focus(); // 첫번째 패스워드로 focus 설정
    ...
    .catch(error => { // 서버 다운등의 통신에러
      console.error('Error:', error);
    });   

    // 처리중 출력
    current_passwd_msg.innerHTML="<img src='/cyclist/images/ani04.gif' style='width: 5%;'>"; // static 기준
...
<c:import url="/menu/top.do" />
  ...
  <form name='frm' id='frm' method='POST' action='./passwd_update.do'>
    ...
    <div class="form-floating mb-1 mt-3" style ="width: 50%; margin:0px auto;">
      <input type='password' class="form-control" name='current_passwd' id='current_passwd' placeholder="현재 패스워드" autofocus="autofocus">
       <label for="current_passwd">현재 패스워드</label>    
    </div>   
    <div id='current_passwd_msg' style="width: 100%; text-align: center;"></div>
    
    <div class="form-floating mb-1 mt-3" style ="width: 50%; margin:0px auto;">
      <input type='password' class="form-control" name=new_passwd id='new_passwd' placeholder="새로운 패스워드">
      <label for="new_passwd">새로운 패스워드</label>    
    </div>   
    
    <div class="form-floating mb-1 mt-3" style ="width: 50%; margin:0px auto;">
      <input type='password' class="form-control" name='new_passwd2' id='new_passwd2' placeholder="현재 패스워드">
